...
/**
 * This class stands for XMLProperties StructType
 * @subpackage Structs
 */
class XMLProperties extends AbstractStructBase
{
    /**
     * The entry
    ...
    /**
     * Constructor method for XMLProperties
     * @uses XMLProperties::setEntry()
    ...
    public static function validateEntryForArrayConstraintsFromSetEntry(?array $values = []): string
    {
        ...
        $message = '';
        $invalidValues = [];
        foreach ($values as $xMLPropertiesEntryItem) {
            // validation for constraint: itemType
            if (!$xMLPropertiesEntryItem instanceof \Conventus\Atoss\StructType\Entry) {
                $invalidValues[] = is_object($xMLPropertiesEntryItem) ? get_class($xMLPropertiesEntryItem) : sprintf('%s(%s)', gettype($xMLPropertiesEntryItem), var_export($xMLPropertiesEntryItem, true));
            }
        }
     ...
     * @param \Conventus\Atoss\StructType\Entry[] $entry
     * @return \Conventus\Atoss\StructType\XMLProperties
     */
